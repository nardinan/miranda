    ...
    if (transition_attributes->drawable) {
        ...
        if (transition_attributes->current_key) {
            if (transition_attributes->next_key) {
                center_x = local_center_x + d_transition_factor_value(transition_attributes->current_key->position_x,
                        transition_attributes->next_key->position_x, fraction_elapsed_update);
                center_y = local_center_y + d_transition_factor_value(transition_attributes->current_key->position_y,
                        transition_attributes->next_key->position_y, fraction_elapsed_update);
                position_x = local_position_x + d_transition_factor_value(transition_attributes->current_key->position_x, 
                        transition_attributes->next_key->position_x, fraction_elapsed_update);
                position_y = local_position_y + d_transition_factor_value(transition_attributes->current_key->position_y, 
                        transition_attributes->next_key->position_y, fraction_elapsed_update);
                zoom = drawable_attributes_self->zoom * d_transition_factor_value(transition_attributes->current_key->zoom, 
                        transition_attributes->next_key->zoom, fraction_elapsed_update);
                angle = drawable_attributes_self->angle + d_transition_factor_value(transition_attributes->current_key->angle, 
                        transition_attributes->next_key->angle, fraction_elapsed_update);
                mask_R = (drawable_attributes_self->last_mask_R / 255.0) * d_transition_factor_value(transition_attributes->current_key->mask_R,
                        transition_attributes->next_key->mask_R, fraction_elapsed_update);
                mask_G = (drawable_attributes_self->last_mask_G / 255.0) * d_transition_factor_value(transition_attributes->current_key->mask_G, 
                        transition_attributes->next_key->mask_G, fraction_elapsed_update);
                mask_B = (drawable_attributes_self->last_mask_B / 255.0) * d_transition_factor_value(transition_attributes->current_key->mask_B, 
                        transition_attributes->next_key->mask_B, fraction_elapsed_update);
                mask_A = (drawable_attributes_self->last_mask_A / 255.0) * d_transition_factor_value(transition_attributes->current_key->mask_A, 
                        transition_attributes->next_key->mask_A, fraction_elapsed_update);
            } else {
...
d_define_class(transition) {
    d_hook_method(transition, e_flag_public, append_key),
