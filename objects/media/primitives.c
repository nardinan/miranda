...
void f_primitive_fill_triangle(SDL_Renderer *renderer, int x_A, int y_A, int x_B, int y_B, int x_C, int y_C, int red, int green, int blue, int alpha) {
  ...
  if (sorted_y[0] > sorted_y[1]) d_swap(0, 1);
  if (sorted_y[1] > sorted_y[2]) d_swap(1, 2);
  if (sorted_y[0] > sorted_y[1]) d_swap(0, 1);
  if (sorted_y[1] == sorted_y[2])
    p_primitive_fill_triangle_bottom_flat(renderer, sorted_x[0], sorted_y[0], sorted_x[1], sorted_y[1], sorted_x[2], sorted_y[2], red, green, blue, alpha);
  else if (sorted_y[0] == sorted_y[1])
    p_primitive_fill_triangle_top_flat(renderer, sorted_x[0], sorted_y[0], sorted_x[1], sorted_y[1], sorted_x[2], sorted_y[2], red, green, blue, alpha);
  else {
    vertex_x = sorted_x[0] + ((double) (sorted_y[1] - sorted_y[0]) / (double) (sorted_y[2] - sorted_y[0])) * (sorted_x[2] - sorted_x[0]);
    vertex_y = sorted_y[1];
    p_primitive_fill_triangle_bottom_flat(renderer, sorted_x[0], sorted_y[0], sorted_x[1], sorted_y[1], vertex_x, vertex_y, red, green, blue, alpha);
    ...
    if ((position_x = (int *) alloca(sizeof(int) * entries))) {
      ...
      for (position_y = top_y; position_y <= bottom_y; ++position_y) {
        ...
        for (index = 0; index < entries; ++index) {
          if (((y[index] < position_y) && (position_y <= y[subindex])) || ((y[subindex] < position_y) && (position_y < y[index]))) {
            position_x[entry++] =
              (int) rint(x[index] + ((double) position_y - y[index]) / ((double) y[subindex] - y[index]) * ((double) x[subindex] - x[index]));
