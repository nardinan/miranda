                        ...
                        if ((result = f_animation_new(d_new(animation), (int)cycles, time_ratio))) {
                            ...
                            while ((d_call(json, m_json_get_string, &string_supply, "sds", "frames", index, "drawable"))) {
                                ...
                                if ((bitmap = d_call(self, m_media_factory_get_bitmap, string_supply, media_factory_attributes->environment))) {
                                    ...
                                    if (bitmap) {
                                        d_call(bitmap, m_drawable_set_maskRGB, (unsigned int)frame_mask_R, (unsigned int)frame_mask_G, 
                                                (unsigned int)frame_mask_B);
...
d_define_method(media_factory, get_json)(struct s_object *self, const char *label) {
    d_using(media_factory);
    struct s_exception *exception;
    struct s_object *stream;
    struct s_object *result = NULL;
...
d_define_method(media_factory, get_script)(struct s_object *self, const char *label) {
    d_using(media_factory);
    struct s_object *stream;
    struct s_object *result = NULL;
    if ((stream = d_call(media_factory_attributes->resources_lisp, m_resources_get_stream_strict, label, e_resources_type_common)))
...
d_define_class(media_factory) {
    d_hook_method(media_factory, e_flag_public, get_bitmap),
        ...
        d_hook_method(media_factory, e_flag_public, get_json),
        d_hook_method(media_factory, e_flag_public, get_script),
