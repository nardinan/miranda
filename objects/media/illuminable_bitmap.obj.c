...
struct s_object *f_illuminable_bitmap_new(struct s_object *self, struct s_object *stream, struct s_object *environment) {
  ...
  f_polygon_new(d_use(&(attributes->polygon_shadow_caster), polygon), 0, NULL);
  f_polygon_new(d_use(&(attributes->polygon_shadow_caster_normalized), polygon), 0, NULL);
  return self;
}
...
d_define_method_override(illuminable_bitmap, draw)(struct s_object *self, struct s_object *environment) {
  ...
  double image_x, image_y, image_center_x, image_center_y, image_width, image_height, image_principal_point_x, image_principal_point_y, new_x, new_y,
    ...
    sine = sin(radians_rotation), light_normalized_percentage_R[e_illuminable_bitmap_side_NULL], light_normalized_percentage_B[e_illuminable_bitmap_side_NULL],
    light_normalized_percentage_G[e_illuminable_bitmap_side_NULL], light_final_mask_R[e_illuminable_bitmap_side_NULL],
    light_final_mask_G[e_illuminable_bitmap_side_NULL], light_final_mask_B[e_illuminable_bitmap_side_NULL],
    light_radius_component[e_illuminable_bitmap_side_NULL], center_factor, side_factor, real_light_x, real_light_y, collision_x = 0, collision_y = 0,
    distance_collision, diagonal_size_squared, distance_center_squared;
