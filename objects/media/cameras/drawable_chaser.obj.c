...
d_define_method_override(drawable_chaser, update)(struct s_object *self, double *screen_position_x, double *screen_position_y, double *screen_w,
  ...
  if (drawable_chaser_attributes->change_position) {
    ...
    if (drawable_chaser_attributes->reference) {
      d_call(drawable_chaser_attributes->reference, m_drawable_get_position, &reference_position_x, &reference_position_y);
      drawable_chaser_attributes->destination_x = reference_position_x;
      drawable_chaser_attributes->destination_y = reference_position_y;
    }
      ...
      if ((camera_controller_attributes->affected_axis & e_camera_controller_axis_x) == e_camera_controller_axis_x) {
        if ((current_distance = (drawable_chaser_attributes->destination_x + drawable_chaser_attributes->offset_x) - (*scene_offset_x)) != 0.0) {
          current_speed = drawable_chaser_attributes->minimum_speed + ((drawable_chaser_attributes->maximum_speed - drawable_chaser_attributes->minimum_speed) *
                                                                       (current_distance / drawable_chaser_attributes->acceleration_distance));
          current_speed = d_math_min(d_math_min(current_speed, drawable_chaser_attributes->maximum_speed), drawable_chaser_attributes->minimum_speed);
          final_position_x = (current_speed * elapsed_time) + (*scene_offset_x);
          if (current_speed > 0) {
            if (final_position_x > (drawable_chaser_attributes->destination_x + drawable_chaser_attributes->offset_x))
              final_position_x = drawable_chaser_attributes->destination_x + drawable_chaser_attributes->offset_x;
          } else if (final_position_x < (drawable_chaser_attributes->destination_x + drawable_chaser_attributes->offset_x))
            final_position_x = drawable_chaser_attributes->destination_x + drawable_chaser_attributes->offset_x;
        }
      } else if ((camera_controller_attributes->affected_axis & e_camera_controller_axis_y) == e_camera_controller_axis_y) {
        if ((current_distance = (drawable_chaser_attributes->destination_y + drawable_chaser_attributes->offset_y) - (*scene_offset_y)) != 0.0) {
          ...
          if (current_speed > 0) {
            if (final_position_y > (drawable_chaser_attributes->destination_y + drawable_chaser_attributes->offset_y))
              final_position_y = drawable_chaser_attributes->destination_y + drawable_chaser_attributes->offset_y;
          } else if (final_position_y < (drawable_chaser_attributes->destination_y + drawable_chaser_attributes->offset_y))
            final_position_y = drawable_chaser_attributes->destination_y + drawable_chaser_attributes->offset_y;
        }
      } else if ((camera_controller_attributes->affected_axis & e_camera_controller_axis_z) == e_camera_controller_axis_z) {
