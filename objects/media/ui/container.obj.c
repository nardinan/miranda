...
d_define_method(container, add_drawable)(struct s_object *self, struct s_object *drawable, double position_x, double position_y, double width, double height,
		t_boolean eventable) {
	d_using(container);
	...
	if ((current_container = (struct s_container_drawable *)d_malloc(sizeof(struct s_container_drawable)))) {
		...
		current_container->width = width;
		current_container->height = height;
		current_container->eventable = eventable;
...
d_define_method_override(container, event)(struct s_object *self, struct s_object *environment, SDL_Event *current_event) {
	...
	d_try {
		d_foreach(&(container_attributes->entries), current_container, struct s_container_drawable)
			if (current_container->eventable)
		...
		if (container_attributes->floatable)
			switch (current_event->type) {
				...
				case SDL_MOUSEBUTTONDOWN:
					if ((current_event->button.button == SDL_BUTTON_LEFT) &&
							(uiable_attributes->selected_mode == e_uiable_mode_selected)) {
...
d_define_method_override(container, draw)(struct s_object *self, struct s_object *environment) {
	...
	d_foreach(&(container_attributes->entries), current_container, struct s_container_drawable) {
		...
		if (current_container->width > d_container_dimension_null)
			dimension_w_entry = current_container->width;
		if (current_container->height > d_container_dimension_null)
			dimension_h_entry = current_container->height;
		...
		if ((d_call(current_container->drawable, m_drawable_normalize_scale, environment_attributes->reference_w[environment_attributes->current_surface],
